...
write.table(tt_mean_std_Data, paste0(workingDir, "test.train_Mean.STD.Data_", format(Sys.Date(), "%Y.%m.%d"), ".txt", collapse = ""), row.names = FALSE)
...
write.table(average_grouped_Data, paste0(workingDir, "test.train.activity_Mean.Summary.Data_", format(Sys.Date(), "%Y.%m.%d"), ".txt", collapse = ""), row.names = FALSE)
